    ...
    def draw_item(
        ...
        if self.layout_type in {"DEFAULT", "COMPACT"}:
            if ob.name.startswith("Transition"):
                layout.prop(
                    ob, "name", text="------", emboss=False, icon_value=layout.icon(ob)
                )
            else:
                layout.prop(
                    ob, "name", text="", emboss=False, icon_value=layout.icon(ob)
...
def ik_control_panel(context, layout, row, path_venv, file_fbx):
    ...
    if scene.ik_advanced:
        ...
        row_ik_adv_row.operator(
            "fdh.start_end__foot_lock", text="L Copy B2E"
        ).option = 4
        row_ik_adv_row.operator(
            "fdh.start_end__foot_lock", text="R Copy B2E"
        ).option = 5
        row_ik_adv.separator()
        row_ik_adv_row = row_ik_adv.row(align=1)
...
class FOURDHUMANS_PT_Panel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        path_venv_wham = join(
            fourd_prop.str_venv_path, fourd_prop.str_custom_venv_name_wham
        )
        path_venv_slahmr = join(
            fourd_prop.str_venv_path, fourd_prop.str_custom_venv_name_slahmr
        )
        flag_installed = join(
            fourd_prop.str_venv_path,
            fourd_prop.str_custom_venv_name,
            "Lib",
            "site-packages",
            "torch",
        )
        flag_installed_wham = join(
            fourd_prop.str_venv_path,
            fourd_prop.str_custom_venv_name_wham,
            "Lib",
            "site-packages",
            "torch",
        )
        flag_installed_wham_reqs = join(
            fourd_prop.str_venv_path,
            fourd_prop.str_custom_venv_name_wham,
            "Lib",
            "site-packages",
        ...
        flag_installed_pre_detectron2 = join(
            fourd_prop.str_venv_path,
            fourd_prop.str_custom_venv_name,
            "Lib",
            "site-packages",
        ...
        flag_installed_detectron2 = join(
            fourd_prop.str_venv_path,
            fourd_prop.str_custom_venv_name,
            "Lib",
            "site-packages",
        ...
        flag_installed_slahmr_torch = join(
            fourd_prop.str_venv_path,
            fourd_prop.str_custom_venv_name_slahmr,
            "Lib",
            "site-packages",
            "torch",
        )
        flag_installed_slahmr_reqs = join(
            fourd_prop.str_venv_path,
            fourd_prop.str_custom_venv_name_slahmr,
            "Lib",
            "site-packages",
        ...
        flag_installed_slahmr_detectron2 = join(
            fourd_prop.str_venv_path,
            fourd_prop.str_custom_venv_name_slahmr,
            "Lib",
            "site-packages",
        ...
        flag_installed_slahmr_lietorch = join(
            fourd_prop.str_venv_path,
            fourd_prop.str_custom_venv_name_slahmr,
            "Lib",
            "site-packages",
        ...
        row_col.prop(
            fourd_prop,
            "bool_extract_marked_frames_panel",
            toggle=True,
            text="Extract Marked Frames",
        )
        row_col = row.row(align=True)
        row_col.prop(
            fourd_prop, "bool_ik_control_panel", toggle=True, text="IK Control"
        )
        if fourd_prop.bool_4dhumans:
            ...
            row_col.prop(
                fourd_prop, "bool_export_panel", toggle=True, text="Export Tools"
            )
            row_col = row.row(align=True)
            row_col.prop(
                fourd_prop, "bool_nla_action_panel", toggle=True, text="NLA Action"
            )
            row_col.prop(
                fourd_prop, "bool_nla_track_panel", toggle=True, text="NLA Track"
            )
            ...
            if fourd_prop.bool_core_panel:
                if fourd_prop.bool_core_exe_panel:
                    ...
                    _label_multiline(context=context, text=text_path, parent=row_text)
                    ...
                    row_exe_run.operator(
                        "fdh.execute", text="Execute w/ Less VRAM"
                    ).option = 1
                ...
                if fourd_prop.bool_core_imp_exp_panel:
                    ...
                    row_anim_exp.operator(
                        "wm.url_open", text="Animation Library", icon="URL"
                    ).url = "https://www.patreon.com/cebstudios/posts?filters%5Btag%5D=4d-humans-animation"
                    ...
                    if os.path.exists(file_pkl):
                        row_anim_exp_buttom.operator(
                            "fdh.export_animation", text="Export"
                        )
                    row_anim_exp_buttom.operator(
                        "fdh.import_pkl_animation", text="Import"
                    ).option = 0
                    row_anim_exp_buttom_col.operator(
                        "fdh.import_pkl_animation", text="Import PKL Google Colab"
                    ).option = 1
                ...
                if fourd_prop.bool_core_smooth_panel:
                    if os.path.exists(file_pkl):
                        ...
                        smooth_trans = row_smooth_col.operator(
                            "fdh.smooth", text="Translation"
                        )
                ...
                if fourd_prop.bool_core_load_panel:
                    ...
                    row_column_pre.label(
                        text="Characters : " + str(fourd_prop.int_tot_character)
                    )
                    ...
                    if (
                        fourd_prop.int_character <= fourd_prop.int_tot_character
                        and os.path.exists(file_fbx)
                        and not fourd_prop.bool_use_selected_character
                    ) or (
                        fourd_prop.bool_use_selected_character
                        and context.scene.source != None
                        and context.scene.source.type == "ARMATURE"
                    ):
                        row_column_import.enabled = True
                    else:
                        row_column_import.enabled = False
                    row_column_import.operator(
                        "fdh.import_character", text="Import Raw"
                    ).option = 0
                    ...
                    row_column_import2.operator(
                        "fdh.import_character", text="Import Smooth"
                    ).option = 1
            ...
            if fourd_prop.bool_footlock_panel:
                ...
                row_flock_sub0.operator(
                    "fdh.foot_lock_marker", text="Floor Reference"
                ).option = 2
                ...
                row_flock.operator(
                    "fdh.foot_lock_marker", text="REMOVE MARKERS"
                ).option = -1
            ...
            if fourd_prop.bool_export_panel:
                ...
                row_export_col.operator(
                    "fdh.remove_location_animation", text="Remove Location"
                )
            ...
            if fourd_prop.bool_nla_action_panel:
                ...
                row_nla_col_paste.operator(
                    "fdh.nla_copy_name_to_nla", icon="PASTEDOWN", text=""
                )
            ...
            if fourd_prop.bool_nla_track_panel:
                ...
                if strips_check:
                    ...
                    row_strip_col_act.operator(
                        "fdh.add_action_to_track_w_transition", text="Add w Transition"
                    ).option = 0
                    ...
                    if (
                        ...
                        and ob.animation_data is not None
                        and len(ob.animation_data.nla_tracks) > 0
                        and len(ob.animation_data.nla_tracks[0].strips) > 0
                        and scn.active_track_index >= 0
                    ):
                        act_type_on_strip = (
                            ob.animation_data.nla_tracks[0]
                            .strips[scn.active_track_index]
                            .type
                        )
                        if scn.active_track_index > 0:
                            act_type_on_strip_prev = (
                                ob.animation_data.nla_tracks[0]
                                .strips[scn.active_track_index - 1]
                                .type
                            )
                        else:
                        ...
                        if (
                            scn.active_track_index
                            < len(ob.animation_data.nla_tracks[0].strips) - 1
                        ):
                            act_type_on_strip_next = (
                                ob.animation_data.nla_tracks[0]
                                .strips[scn.active_track_index + 1]
                                .type
                            )
                    ...
                    if (ob is not None and scn.active_track_index == 0) or (
                        ob is not None
                        and scn.active_track_index is not None
                        and (
                            act_type_on_strip == "TRANSITION"
                            or act_type_on_strip_prev == "TRANSITION"
                        )
                    ):
                    ...
                    if (
                        ...
                        and ob.animation_data is not None
                        and len(ob.animation_data.nla_tracks) > 0
                        and len(ob.animation_data.nla_tracks[0].strips) - 1
                        == scn.active_track_index
                    ) or (
                        ob is not None
                        and ob.animation_data is not None
                        ...
                        and (
                            act_type_on_strip == "TRANSITION"
                            or act_type_on_strip_next == "TRANSITION"
                        )
                    ):
                    ...
                    row_strip_col_trans_lt.operator(
                        "fdh.add_action_to_track_w_transition",
                        icon="TRIA_LEFT",
                        text="",
                    ).option = 1
                    row_strip_col_trans_gt.operator(
                        "fdh.add_action_to_track_w_transition",
                        icon="TRIA_RIGHT",
                        text="",
                    ).option = 2
                    ...
                    if nla_window == 1:
                        ...
                        if fourd_prop.bool_nla_selection_tools:
                            ...
                            row_strip_col_select.operator(
                                "fdh.select_action_on_strip", text="All"
                            ).option = 1
                            row_strip_col_select.operator(
                                "fdh.select_action_on_strip", text="None"
                            ).option = 2
                    row_strip.operator("fdh.match_to_prev_action")

                    if not strips_check:
                        rows = 4
                        row_strip.template_list(
        ...
        if fourd_prop.bool_retarget_panel:
            ...
            if (
                armature_mesh is not None
                and armature_mesh.type == "MESH"
                and armature_mesh.data.shape_keys is not None
            ):  # shape_keys is not nonee para ver se tem shapekeys
                _label_multiline(
                    context=context,
            ...
            if (
                scn.target is not None
                and scn.target.get("bind") is not None
                and scn.target["bind"] == 1
            ):
                row_remap_unbind.enabled = True
            else:
                row_remap_unbind.enabled = False
            row_remap_unbind.operator("fdh.retarget", text="Unbind").option = (
                1  # unbind
            )
            ...
            row_remap_apply_retarget.operator(
                "fdh.retarget", text="Apply Retarget"
            ).option = 2  # apply
        ...
        if fourd_prop.bool_extract_marked_frames_panel:
            ...
            rowb_extract_marker.operator(
                "fdh.extract_marked_frames", text="Entire Character"
            )
            if (
                context.active_object is not None
                ...
                rowb_extract_marker.operator(
                    "fdh.extract_marked_frames_for_selected_bones"
                )
        ...
        if fourd_prop.bool_wham:
            ...
            _label_multiline(context=context, text=text_path, parent=row)
            ...
            row_export_pkl_col_export.operator(
                "fdh.export_animation", text="Export"
            ).option = 1  # 1 for WHAM
            row_export_pkl_col.operator(
                "fdh.import_pkl_animation", text="Import"
            ).option = 2  # 2 for wham
            row_export_pkl.operator(
                "fdh.import_pkl_animation", text="Import Google Colab PKL"
            ).option = 3  # 2 for wham
            ...
            row_import_character_col.label(
                text="Total Char: " + str(fourd_prop.int_tot_character)
            )
            ...
            if (
                fourd_prop.bool_world
            ):  # se local, remover oo movimento Z (deixar a opcao)
            ...
            row_import_character.prop(
                fourd_prop, "bool_world", text=world_local_w_text, toggle=1
            )
        ...
        if fourd_prop.bool_slahmr:
            ...
            row.label(
                text=str(
                    fourd_prop.int_end_frame_slahmr - fourd_prop.int_ini_frame_slahmr
                )
                + " Frames"
            )
            row_col = row.row(align=1)
            ...
            row_execute_slahmr.operator("fdh.import_character").option = (
                3  # importa slahmr
            )
        ...
        if fourd_prop.bool_4dhumans:
            ...
            if scene.fourd_venv:
                ...
                if fourd_prop.bool_custom_venv:
                    ...
                    row_subpath.prop(
                        fourd_prop, "bool_custom_venv_name", text="Name", toggle=1
                    )
                    row_subpath.prop(
                        fourd_prop,
                        "bool_custom_venv_name_from_list",
                        text="List",
                        toggle=1,
                    )
                    ...
                    if fourd_prop.bool_custom_venv_name:
                        ...
                        row_sub_subpath.prop(
                            fourd_prop, "str_custom_venv_name", text="Name"
                        )
                    if fourd_prop.bool_custom_venv_name_from_list:
                        row_sub_subpath = row.column(align=True)
                        # row_subpath = row_subpath.column(align=True)
                        row_sub_subpath.prop(
                            fourd_prop, "enum_custom_venv_list", text="Folder"
                        )
                ...
                if os.path.exists(
                    flag_installed_cv2
                ):  # and os.path.exists(flag_installed_einops):
                ...
                if " " in path_venv:
                    ...
                    row_alert_install_pre_detectron2.operator(
                        "fdh.install_pre_detectron_venv"
                    )
                    ...
                    row_alert_smpl_pkl.operator(
                        "fdh.import_smpl_pkl",
                        text="Import basicModel_neutral_lbs_10_207_0_v1.0.0.pkl",
                    ).option = 3
                ...
                row.operator(
                    "wm.url_open", text="MS Visual C++ Redistributable", icon="URL"
                ).url = "https://aka.ms/vs/16/release/vc_redist.x64.exe"
                row = layout.row().box()
                row = row.column(align=True)
                row.label(text="Official Github")
                row.operator("wm.url_open", text="4d Humans Github", icon="URL").url = (
                ...
                row.operator("wm.url_open", text="SMPL Page", icon="URL").url = (
                ...
                row.operator("wm.url_open", text="SMPLify Page", icon="URL").url = (
        ...
        if fourd_prop.bool_wham:
            ...
            if scene.fourd_venv_wham:
                ...
                if (
                    os.path.exists(flag_ckp_dpvo_pth)
                    and os.path.exists(flag_ckp_hmr2a_ckpt)
                    and os.path.exists(flag_ckp_vitpose)
                    and os.path.exists(flag_ckp_wham_vit_w)
                    and os.path.exists(flag_ckp_wha_vit_bedlam)
                    and os.path.exists(flag_ckp_yolov8x)
                ):
                ...
                if " " in path_venv_wham:
                    ...
                    row_alert_install_pytorch_wham.operator(
                        "fdh.install_pytorch_venv_wham"
                    )
                    row_alert_install_wham.operator("fdh.install_wham_venv")
                    row_alert_install_bodymodels.operator(
                        "fdh.download_whan_bodymodels"
                    )

                    row_alert_install_checkpoints = (
                        row_alert_install_checkpoints.column(align=True)
                    )
                    ...
                    r_a_i_ckpt_box_col.operator(
                        "fdh.download_whan_checkpoints", text="Online"
                    )
                    r_a_i_ckpt_box_col.operator(
                        "fdh.import_offline_checkpoint_wham", text="Offline"
                    )
                ...
                row = row.column(align=True)
                row.label(text="Official Github")
                row.operator("wm.url_open", text="WHAM Github", icon="URL").url = (
                ...
                row.operator(
                    "wm.url_open", text="Download SMPL FBX", icon="DISK_DRIVE"
                ).url = "https://download.is.tue.mpg.de/download.php?domain=smpl&sfile=SMPL_maya.zip"
                ...
                row.operator(
                    "wm.url_open", text="Download SMPL Neutral", icon="DISK_DRIVE"
                ).url = "https://download.is.tue.mpg.de/download.php?domain=smplify&resume=1&sfile=mpips_smplify_public_v2.zip"
                row_sub = row.column(align=True)
                ...
                row_alert_install_smpl_neutral.label(
                    text="Neutral PKL - basicModel_neutral_lbs_10_207_0_v1.0.0.pkl"
                )
                row_alert_install_smpl_neutral.operator(
                    "fdh.import_smpl_pkl", text='Import "basicmodel_neutral" Neutral '
                ).option = 2
        ...
        if fourd_prop.bool_slahmr:
            ...
            if scene.fourd_venv_slahmr:
                ...
                if " " in path_venv_slahmr:
                    ...
                    row_alert_create_venv_slahmr.operator(
                        "fdh.create_virtual_env_slahmr"
                    )
                    row_alert_install_pytorch_slahmr.operator(
                        "fdh.install_pytorch_venv_slahmr"
                    )
                    row_alert_install_slahmr_detectron.operator(
                        "fdh.install_detectron_venv_slahmr"
                    )
                    row_alert_install_slahmr_reqs.operator(
                        "fdh.install_slahmr_venv_reqs"
                    )
                    row_alert_install_slahmr_rest_venv.operator(
                        "fdh.install_slahmr_and_rest_venv"
                    )
                row = layout.row().box()
                row = row.column(align=True)
                row.operator(
                    "wm.url_open",
                    text="Download SLAHMR Dependencies",
                    icon="DISK_DRIVE",
                ).url = (
                    "https://drive.google.com/uc?id=1GXAd-45GzGYNENKgQxFQ4PHrBp8wDRlW"
                )
                ...
                row = row.column(align=True)
                row.label(text="Official Github")
                row.operator("wm.url_open", text="SLAHMR Github", icon="URL").url = (
...
class PIXELMySettings(PropertyGroup):
    ...
    str_custom_venv_name_wham: StringProperty(
        name="Name", default=DEFAULT_NAME_WHAM, update=updt_custom_venv_name
    )
    str_custom_venv_name_slahmr: StringProperty(
        name="Name", default=DEFAULT_NAME_SLAHMR, update=updt_custom_venv_name
    )
    ...
    bool_use_selected_character: BoolProperty(
        name="Use selected character", default=False
    )
    ...
    bool_extract_marked_frames_panel: BoolProperty(
        name="Show Extract Marked Frames Panel", default=False
    )
    bool_ik_control_panel: BoolProperty(name="Show IK Control Panel", default=True)

    bool_nla_selection_tools: BoolProperty(
        name="Show NLA Selection tools Panel", default=False
    )
    bool_retarget_hide_source: BoolProperty(
        name="Hide Source After Retarget", default=True
    )
    ...
    ### Floor setup
    fl_floor_distance_fine_tune: FloatProperty(
        name="Floor Fine Tune", default=0.061255, update=floor_distance_fine_tune
    )
    ...
    bool_clear_before_load_quickload: BoolProperty(
        name="Clear before load", default=True
    )
