...
def bind(context, rig, target):
    ...
    for b in rig:
        if scn.source.name.startswith("STG2_Armature") and b[0] in [
            "m_avg_root",
            "m_avg_L_Hand",
            "m_avg_R_Hand",
        ]:
        ...
        if b[1] != "":
            ...
            copy_bone_dest = scn.source.data.edit_bones.new(
                bone_dest.name + "_CP_CEB4D"
            )
    ...
    for b in rig:
        if b[1] != "":
            if target == "stg2":
                cp_rot = scn.target_stg2.pose.bones[b[1]].constraints.new(
                    "COPY_ROTATION"
                )
            else:
        ...
        if b[0] == "m_avg_Pelvis":
            ...
            if target == "stg2":
                cp_loc = scn.target_stg2.pose.bones["Ctrl_Hips"].constraints.new(
                    "COPY_LOCATION"
                )
            else:
...
def retarget(context, target):
    ...
    if target == "stg2":
        bpy.ops.nla.bake(
            ...
            frame_end=end_frame,
            only_selected=fourd_prop.bool_selected_bones,
            visual_keying=True,
...
class CreateVirtualEnviroment(Operator):
    ...
    def execute(self, context):
        ...
        if os.path.exists(join(path_venv, fourd_prop.str_custom_venv_name)):
            ...
            subprocess.run(
                [
                    join(path_addon, "install_venv_p310.bat"),
                    drive_env,
                    path_addon,
                    path_venv,
                    env_folder_name,
...
class CreateVirtualEnviromentWHAM(Operator):
    ...
    def execute(self, context):
        ...
        if os.path.exists(join(path_venv, fourd_prop.str_custom_venv_name_wham)):
            ...
            subprocess.run(
                [
                    join(path_addon, "install_venv_p39.bat"),
                    drive_env,
                    path_addon,
                    path_venv,
                    env_folder_name_wham,
                ]
            )  # Cria o venv
...
class InstallPYTORCHOnVenvWHAM(Operator):
    ...
    def execute(self, context):
        ...
        # Executa oprocesso
        os.system(
            'start cmd /k ""' + join(path_addon, "install_pytorch_wham.bat") + '" "'
        )
...
class InstallPREDETECTRONOnVenv(Operator):
    ...
    def execute(self, context):
        ...
        # Executa oprocesso
        os.system(
            'start cmd /k ""' + join(path_addon, "install_pre_detectron.bat") + '" "'
        )
...
class InstallREQPIXELOnVenv(Operator):
    ...
    def execute(self, context):
        ...
        fourd_prop = context.scene.fourd_prop
        path_venv = join(
            fourd_prop.str_venv_path, fourd_prop.str_custom_venv_name, "src"
        )
        ...
        os.system(
            'start cmd /k ""'
            + join(path_addon, "install_SDM_reqs.bat")
            + '" '
            + drive
            + " "
            + rest_path
...
class ImportSMPLPKL(Operator, ImportHelper):
    ...
    def execute(self, context):
        ...
        if self.option == 3:  # for 4d humans
            ...
            dst2 = os.path.join(
                path_addon, "basicModel_neutral_lbs_10_207_0_v1.0.0_p3.pkl"
            )
...
class ImportVideo(Operator, ImportHelper):
    ...
    def execute(self, context):
        ...
        with open(
            os.path.join(
                path_addon, "4D-Humans-main", "PHALP", "phalp", "configs", "base_ref.py"
            ),
            "rt",
        ) as fin:
            with open(
                os.path.join(
                    path_addon, "4D-Humans-main", "PHALP", "phalp", "configs", "base.py"
                ),
                "wt",
            ) as fout:
                for line in fin:
...
class ImportVideoSLAHMR(Operator, ImportHelper):
    ...
    def execute(self, context):
        ...
        with open(
            os.path.join(
                path_addon, "slahmr", "slahmr", "confs", "data", "video_ref.yaml"
            ),
            "rt",
        ) as fin:
            with open(
                os.path.join(
                    path_addon, "slahmr", "slahmr", "confs", "data", "video.yaml"
                ),
                "wt",
            ) as fout:
                for line in fin:
...
class LoadVideo3dview(Operator):
    ...
    def execute(self, context):
        ...
        bpy.ops.import_image.to_plane(
            files=[{"name": vid_name, "name": vid_name}],
            directory=vid_dir,
            relative=False,
        )
...
class Execute(Operator):
    ...
    option: IntProperty(
        name="type of mask", default=0
    )  # 0=vitdet more vram 14.5gb, 1=maskrcnn less vram about 8.5gb
...
class ImportCharacter(Operator):
    ...
    def execute(self, context):
        ...
        def Rodrigues(rotvec):
            ...
            cost = np.cos(theta)
            mat = np.asarray(
                [[0, -r[2], r[1]], [r[2], 0, -r[0]], [-r[1], r[0], 0]], dtype=object
            )  # adicionei "",dtype=object" por que estava dando erro
        ...
        def get_global_pose(global_pose, arm_ob, frame=None):
            ...
            rot_world_orig = (
                root_orig @ pelvis_applyied @ mw_orig
            )  # pegar a rotacao em relacao ao mundo
        ...
        def apply_trans_pose_shape(trans, body_pose, arm_ob, obname, frame=None):
            ...
            if self.option == 2:  # pro WHAM
                if fourd_prop.bool_world:
                    ...
                    if fourd_prop.bool_fix_z:
                        trans = Vector(
                            (trans[0], trans[1] - 2.2, 0)
                        )  # o -2 é para tentar colocar o personagem no chao ao inves de ficar sob o chao
                    else:
            ...
            if self.option == 2:  # rotaciona para WHAM
                ...
                if fourd_prop.bool_world:
                    arm_ob.pose.bones["m_avg_root"].rotation_quaternion = Vector(
                        (1.0, 0.0, 0.0, 0.0)
                    )
                    arm_ob.pose.bones["m_avg_root"].keyframe_insert(
            ...
            for ibone, mrot in enumerate(mrots):
                ...
                if fourd_prop.bool_use_selected_character:
                    ...
                    bone = arm_ob.pose.bones[
                        obname + "_" + part_bones["bone_%02d" % ibone]
                    ]
        ...
        def init_scene(scene, params, gender="male", angle=0):
            ...
            if fourd_prop.bool_use_selected_character:
                ...
                bpy.ops.import_scene.fbx(
                    filepath=path_fbx, axis_forward="-Y", axis_up="-Z", global_scale=100
        ...
        if self.option == 2:  # para importar o WHAM
            ...
            for fframe in range(0, len(frames_ids)):
                if character <= len(results) - 1:
                    real_frame = (
                        frames_ids[0] + fframe
                    )  # corrigindo o frame real para poder aparecer corretamento no timeline
                    # print('fframe: ',fframe)
                    ...
                    trans = results[int(list_characters[character])][
                        "trans" + bool_pose
                    ][fframe]
                    ...
                    final_body_pose = results[int(list_characters[character])][
                        "pose" + bool_pose
                    ][fframe]
                    apply_trans_pose_shape(
                        Vector(trans), final_body_pose, arm_ob, obname, real_frame
                    )
                    bpy.context.view_layer.update()
                else:
        ...
        if self.option in [0, 1]:  # 4d humans com e sem smoothnet
            ...
            for fframe, data in enumerate(results.items()):
                if character <= len(data[1]["smpl"]) - 1:
                    ...
                    apply_trans_pose_shape(
                        Vector(trans), final_body_pose, arm_ob, obname, fframe
                    )
                    bpy.context.view_layer.update()
                else:
                    print("skipping to the next")
        ...
        if not fourd_prop.bool_use_selected_character:
            ...
            if self.option == 2:  # para importar o WHAM
                ...
                arm_ob.name = (
                    txt_amt + "_Amt_C" + str(fourd_prop.int_character).zfill(2)
                )
            else:
...
class Smooth(Operator):
    ...
    pkl_file: StringProperty(
        name="pkl_file", default="demo_video_converted.pkl"
    )  # em geral 'demo_video_converted.pkl' 1='demo_video_converted_smooth.pkl'
    ...
    def execute(self, context):
        ...
        with open(join(path_addon, batch_file), "wt") as fout:
            ...
            # fout.write('\ncd 4D-Humans-main')
            fout.write(
                "\npython smooth.py --tot_characters "
                + str(fourd_prop.int_tot_character)
                + " --pkl_file_input "
                + self.pkl_file
                + " --smooth_what "
                + self.smooth_what
...
class ExportRAWAnimation(bpy.types.Operator, ExportHelper):
    ...
    option: IntProperty(
        name="Export PKL", default=0
    )  # 0 = CEB 4d Humans PKL  | 1 = WHAM
    ...
    def execute(self, context):
        ...
        if self.option == 0:
            source_folder = os.path.join(
                path_addon, "slahmr", "outputs", "video_slahmr"
            )
...
class ReadPKLData(Operator):
    ...
    option: IntProperty(
        name="Chosse 4d humans / WHAM", default=0
    )  # 0 = CEB 4d Humans PKL (Pickle) | 1 = 4d Humans PKL (joblib)
...
class Smooth2(Operator):
    ...
    option: IntProperty(
        name="Type of Smooth", default=0
    )  # 0= bake and smooth, 1= only bake,2=only smooth
...
class FootLockMarker(Operator):
    ...
    option: IntProperty(
        name="Start/Stop Foot Lock", default=0
    )  # -1=apagra markes; 0=Foot lock start, 1= Foot lock end; 2=set default height
...
class FootLock(Operator):
    ...
    option: IntProperty(
        name="analyze and execute Foot Lock", default=0
    )  # 0=analyze foot lock, 1= place foot locks, 2=original location
    ...
    def execute(self, context):
        ...
        if self.option == 0:
            ...
            for m in bpy.context.scene.timeline_markers:
                if m.name == "Ini_height":
                    marker_ini_height = (
                        m.frame
                    )  # espera-se que tnha apenas um ini heigh
                    break
            ...
            if marker_ini_height is not None:
                scene.frame_set(
                    marker_ini_height
                )  # vai para o frame marcado como referencia
        ...
        if self.option == 1:
            ...
            for ef, f in enumerate(range(scene.frame_start, scene.frame_end + 1)):
                ...
                #
                if foot_lock == 1:
                    armature.pose.bones["m_avg_Pelvis"].location[0] = adjust_heigh[ef][
                        1
                    ]
                    armature.pose.bones["m_avg_Pelvis"].location[1] = adjust_heigh[ef][
                        5
                    ]
                    armature.pose.bones["m_avg_Pelvis"].keyframe_insert(
                        "location", frame=f
                    )
...
class ImportOfflineFiles(Operator, ImportHelper):
    ...
    bl_description = (
        "Download first the '4d_humans_optional_offline_model_files.zip' file."
    )
    ...
    def execute(self, context):
        ...
        home = expanduser(
            "~"
        )  # se nao me engano isso vai jogar pro caminho "usuario" do windows
...
class RemoveLocationAnimation(Operator):
    ...
    def execute(self, context):
        ...
        fcurves = [
            ...
            if fc.data_path == 'pose.bones["m_avg_Pelvis"].location'
        ]
...
class CopyNameToNLA(Operator):
    ...
    def execute(self, context):
        ...
        if fourd_prop.str_videopath != "":
            fourd_prop.str_nla_name = os.path.splitext(
                os.path.basename(fourd_prop.str_videopath)
            )[0]
        elif fourd_prop.str_pklpath != "":
            fourd_prop.str_nla_name = os.path.splitext(
                os.path.basename(fourd_prop.str_pklpath)
            )[0]
        else:
...
class CreateNLAStrip(Operator):
    ...
    def execute(self, context):
        ...
        if (
            len(armature.animation_data.nla_tracks) > 0
            and len(armature.animation_data.nla_tracks[0].strips) > 0
        ):
...
class AddTrack(Operator):
    ...
    def execute(self, context):
        ...
        context.scene.active_track_index = (
            len(armature.animation_data.nla_tracks[0].strips) - 1
        )
...
class AddActionToTrack(Operator):
    ...
    def execute(self, context):
        ...
        if qtd_actions == 0:
            ...
            action_end = (
                armature.animation_data.nla_tracks[0].strips[qtd_actions - 1].frame_end
            )
        action_to_add = bpy.data.actions[act_idx]

        armature.animation_data.nla_tracks[0].strips.new(
            action_to_add.name, int(action_end), action_to_add
        )
...
class AddActionToTrackWTransition(Operator):
    ...
    option: IntProperty(
        name="Trasition option", default=0
    )  # 0= adiciona com uma nova acao, 1 = adiciona com a acao anterior; 2=adiciona com a acao seguinte
    ...
    def execute(self, context):
        ...
        if self.option == 0:  # adiciona com nova acao apos a ultima acao do nla
            ...
            action_to_add = bpy.data.actions[act_idx]
            armature.animation_data.nla_tracks[0].strips.new(
                action_to_add.name, int(action_end_w_transition), action_to_add
            )
            with bpy.context.temp_override(area=areas3d[0]):
        ...
        if self.option == 1:  # adiciona transition em cima da acao anterior
            ...
            for f_trk in range(trck_idx - 1, qtd_actions):
                ...
                act_in_strip.append(
                    [f_trk, f_start, f_end, f_start + f_trans, f_end + f_trans]
                )

            for f in reversed(
                act_in_strip
            ):  # preenche de tras pra frente, senao nao funciona
                if f[0] >= trck_idx:
...
class RemoveActionFromtrack(Operator):
    ...
    def poll(cls, context):
        ...
        if (
            ob is not None
            and hasattr(ob.animation_data, "nla_tracks")
    ...
    def execute(self, context):
        ...
        armature.animation_data.nla_tracks[0].strips.remove(
            armature.animation_data.nla_tracks[0].strips[idx]
        )
        ...
        while organizar > 0:
            ...
            for f_trk in range(0, qtd_actions):
                ...
                if f_trk == 0:
                    ...
                    act_in_strip.append(
                        [f_trk, f_start, f_end, cur_start, cur_end, cur_end - cur_start]
                    )
            ...
            if (
                f_start_primeiro < 0
            ):  # primeiro empurra tudo pra frente pra passar do negativo no primeiro action
                for f in reversed(act_in_strip):
                    # arm_track.strips[f[0]].frame_start = f[3]
                    # arm_track.strips[f[0]].frame_end = f[4]
                    arm_track.strips[f[0]].frame_start = arm_track.strips[
                        f[0]
                    ].frame_start + abs(f_start_primeiro)
                    arm_track.strips[f[0]].frame_end = arm_track.strips[
                        f[0]
                    ].frame_end + abs(f_start_primeiro)
...
class SelectActionOnStrip(Operator):
    ...
    option: IntProperty(
        name="Select on NLA editor", default=0
    )  # 0= selectiona uma action, 1=seleciona todas as actions no strip, 2=desmarca todas
    ...
    def execute(self, context):
        ...
        if self.option in [
            0,
            2,
        ]:  # se for pra selecionar apenas uma ou desmarcar todas, rodar o desmarc todas
            areas3d = [area for area in scr.areas if area.type == "NLA_EDITOR"]
            with bpy.context.temp_override(area=areas3d[0]):
                bpy.ops.nla.select_all(action="DESELECT")
        ...
        if (
            self.option == 0
        ):  # Seleciona apenas uma (ates desmarca todas no if anterior)
            arm_track.strips[trck_idx].select = True  # seleciona o action atual
...
class MatchToPrevAction(Operator):
    ...
    def execute(self, context):
        ...
        fcurves = [
            fc
            for fc in get_last_action.fcurves
            if fc.data_path == 'pose.bones["m_avg_Pelvis"].location'
        ]
        print("start")
        ...
        fcurves_quat = [
            fc
            for fc in get_last_action.fcurves
            if fc.data_path == 'pose.bones["m_avg_Pelvis"].rotation_quaternion'
        ]
        ...
        fcurves = [
            fc
            for fc in action_copy.fcurves
            if fc.data_path == 'pose.bones["m_avg_Pelvis"].location'
        ]

        cur_x = (
            fcurves[0].keyframe_points[0].co.y
        )  # da acao atual tenho que pegar o primeiro frame
        ...
        for f in range(
            int(action_copy.frame_range[0]), int(action_copy.frame_range[1])
        ):
            fcurves[0].keyframe_points[f].co.y = (
                fcurves[0].keyframe_points[f].co.y + dif_x
            )
            fcurves[1].keyframe_points[f].co.y = (
                fcurves[1].keyframe_points[f].co.y + dif_y
            )
            fcurves[2].keyframe_points[f].co.y = (
                fcurves[2].keyframe_points[f].co.y + dif_z
            )

            fcurves_quat[0].keyframe_points[f].co.y = (
                fcurves_quat[0].keyframe_points[f].co.y + dif_q_w
            )
            fcurves_quat[1].keyframe_points[f].co.y = (
                fcurves_quat[1].keyframe_points[f].co.y + dif_q_x
            )
            fcurves_quat[2].keyframe_points[f].co.y = (
                fcurves_quat[2].keyframe_points[f].co.y + dif_q_y
            )
            fcurves_quat[3].keyframe_points[f].co.y = (
                fcurves_quat[3].keyframe_points[f].co.y + dif_q_z
            )
    ...
    def execute(self, context):
        ...
        for f in range(frame_start, frame_end + 1):
            for bone in context.selected_pose_bones:
                ...
                for fc in fcurves:
                    if (
                        bone.name == fc.data_path.split('"')[1]
                    ):  # verifica se o nome do bone é o mesmo, para poder limpar o ik_switch
                        if fc.data_path.endswith('["ik_fk_switch"]'):
                            kpoints = fc.keyframe_points.values()
                            for kp in kpoints:
                                if f not in markers and f == int(kp.co[0]):
...
class StartChangeRestTgtPose(Operator):
    ...
    def execute(self, context):
        ...
        armature_target.data.pose_position = (
            "POSE"  # nao e possivel mudar a pose se POSE estiver como REST
        )
...
def ik_to_fk_arm(self):
    ...
    if constraint and valid_constraint:
        ...
        if parent_type == "object":
            ...
            hand_ik.matrix = (
                constraint.inverse_matrix.inverted()
                @ obj_par.matrix_world.inverted()
                @ hand_fk.matrix
            )
    else:
...
def switch_ik_fk_anim(self, context, bones):
    ...
    self.frame_start, self.frame_end = int(action.frame_range[0]), int(
    ...
    def execute(self, context):
        ...
        for ob in bpy.data.objects:
            if (
                ...
                and ob.name.startswith("STG2_Armature")
            ):
                stg2_armature = ob
...
class Setupfloor(Operator):
    ...
    def execute(self, context):
        ...
        child_collision = ["Ctrl_Foot_IK_Left", "Ctrl_Foot_IK_Right"]
...
class SetupFootLock(Operator):
    ...
    def execute(self, context):
        ...
        o_r_loc = scn.target_stg2.pose.bones[
            "Ctrl_Foot_IK_Right"
        ].matrix.to_translation()
        o_l_loc = scn.target_stg2.pose.bones[
            "Ctrl_Foot_IK_Left"
        ].matrix.to_translation()

        o_r_rot = scn.target_stg2.pose.bones[
            "Ctrl_Foot_IK_Right"
...
class StartEndFootLock(Operator):
    ...
    option: IntProperty(
        name="", default=0
    )  # 0- start_left; 1-end_left; 2-start_right; 3-end_right; 4=Copy left bone 2 empty loc/rot to empty; 5=Copy right bone 2 empty loc/rot to empty
    ...
    def execute(self, context):
        ...
        if self.option == 0:  # 0- start_left
            ...
            o_l_rot_euler = scn.target_stg2.pose.bones[
                "Ctrl_Foot_IK_Left"
            ].matrix.to_euler()
            o_l_rot_euler.x = 0
            o_l_rot_euler.y = 0
            ...
            ## left
            scn.foot_lock_left.keyframe_insert(
                data_path="location",
                frame=scn.frame_current - fourd_prop.int_fadein_foot_lock_frames,
            )
            scn.foot_lock_left.keyframe_insert(
            ...
            scn.foot_lock_left.keyframe_insert(
                data_path="location", frame=scn.frame_current
            )
            scn.foot_lock_left.keyframe_insert(
                data_path="rotation_quaternion", frame=scn.frame_current
            )
            scn.target_stg2.pose.bones["Ctrl_Foot_IK_Left"].constraints[
                2
            ].influence = 0.0
            scn.target_stg2.pose.bones["Ctrl_Foot_IK_Left"].constraints[
                2
            ...
            scn.target_stg2.pose.bones["Ctrl_Foot_IK_Left"].constraints[
                2
            ].keyframe_insert(data_path="influence", frame=scn.frame_current)
        ...
        if self.option == 2:  # 2-start_right;
            ...
            o_r_rot_euler = scn.target_stg2.pose.bones[
                "Ctrl_Foot_IK_Right"
            ].matrix.to_euler()
            o_r_rot_euler.x = 0
            o_r_rot_euler.y = 0
            ...
            ## right
            scn.foot_lock_right.keyframe_insert(
                data_path="location",
                frame=scn.frame_current - fourd_prop.int_fadein_foot_lock_frames,
            )
            scn.foot_lock_right.keyframe_insert(
            ...
            scn.target_stg2.pose.bones["Ctrl_Foot_IK_Right"].constraints[
                2
            ].influence = 0.0
            scn.target_stg2.pose.bones["Ctrl_Foot_IK_Right"].constraints[
                2
        ...
        if self.option == 1:  # 1-end_left
            ...
            scn.target_stg2.pose.bones["Ctrl_Foot_IK_Left"].constraints[
                ...
                data_path="influence",
                frame=scn.frame_current - fourd_prop.int_fadeout_foot_lock_frames,
            )
            scn.target_stg2.pose.bones["Ctrl_Foot_IK_Left"].constraints[
                2
            ].influence = 0.0
    ...
    def execute(self, context):
        ...
        if self.option == 0:  # left
            ...
            for f in range(start_frame, end_frame + 1):
                ...
                scn.target_stg2.pose.bones["Ctrl_Foot_IK_Left"].constraints[
                    2
                ].keyframe_delete(data_path="influence", frame=f)
                scn.target_stg2.pose.bones["Ctrl_Foot_IK_Left"].constraints[
                    2
                ].influence = 0.0
