...
parser.add_argument(
    "--pkl_file_input", type=str, required=True
)  # normalmente vai ser 'demo_video_converted.pkl' ou 'demo_video_converted_smooth.pkl'
...
for num_character in range(0, characters):
    ...
    if args.smooth_what == "trans" or args.smooth_what == "both":
        ...
        new_trans_0 = smooth_process(
            t0[:, np.newaxis],
            # smooth_type='smoothnet_windowsize8',
            smooth_type=smooth_type_s,
            cfg_base_dir="configs_cliff/_base_/post_processing/",
        ).reshape(frame_num, 3)
    ...
    if args.smooth_what == "pose" or args.smooth_what == "both":
        ...
        new_pose_0 = smooth_process(
            p0.reshape(frame_num, 24, 3),
            # smooth_type='smoothnet_windowsize8',
            smooth_type=smooth_type_s,
            cfg_base_dir="configs_cliff/_base_/post_processing/",
        ).reshape(frame_num, 72)

    # start from 1, the interval is 2
    if args.smooth_what == "trans" or args.smooth_what == "both":
        t1 = trans[1::2]
        frame_num = t1.shape[0]
        new_trans_1 = smooth_process(
            t1[:, np.newaxis],
            # smooth_type='smoothnet_windowsize8',
            smooth_type=smooth_type_s,
            cfg_base_dir="configs_cliff/_base_/post_processing/",
    ...
    if args.smooth_what == "pose" or args.smooth_what == "both":
        ...
        new_pose_1 = smooth_process(
            p1.reshape(frame_num, 24, 3),
            # smooth_type='smoothnet_windowsize8',
            smooth_type=smooth_type_s,
            cfg_base_dir="configs_cliff/_base_/post_processing/",
        ).reshape(frame_num, 72)
    ...
    for ffnew, data_new in enumerate(b.items()):
        if num_character <= len(data_new[1]["smpl"]) - 1:
            ...
            if args.smooth_what == "pose" or args.smooth_what == "both":
                ...
                b[data_new[0]]["smpl"][num_character][
                    "global_orient"
                ] = global_orient_new
